...
export default function TpCamera() {
  ...
  const { camera, set, size } = useThree(({ camera, set, size }) => ({
    camera,
    set,
    size,
  }))
  ...
  useFrame(() => {
    ...
    if (position && camera) {
      ...
      const wantedPosition = new Vector3(
        position.x - prevMovement.current.x,
        position.y + Math.max(1, -prevMovement.current.y * 2),
        position.z - prevMovement.current.z
      )
